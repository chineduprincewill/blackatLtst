...
import { setProgress } from '@state/slices/profileSlice';
...
export const profileApiSlice = apiSlice.injectEndpoints({
    endpoints: (builder) => ({
        ...
        getProfile: builder.mutation<ProfileResponse, { id?: string; userId?: string } | undefined>({
            query: (props) => ({
                url: PROFILE_URL + '/detail',
                method: 'GET',
                params: props ? { id: props.id, userId: props.userId } : undefined,
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: 'Bearer ' + localStorage.getItem('accessToken'),
                },
        ...
        getProfileQuery: builder.query<ProfileResponse, { id?: string; userId?: string } | undefined>({
            query: (props) => ({
                url: PROFILE_URL + '/detail',
                method: 'GET',
                params: props ? { id: props.id, userId: props.userId } : undefined,
        ...
        uploadImageToCloud: builder.mutation<ProfileResponse, { url: string; file: File }>({
            query: ({ url, file }) => {
        ...
        addExperiences: builder.mutation<ExperienceResponse, Omit<Experience, 'id'>[]>({
            query: (experiences) => ({
        ...
        updateExperience: builder.mutation<UpdateExperienceResposne, { id: string; data: Partial<Experience> }>({
            query: ({ id, data }) => {
                console.log({ data, id });
                return {
        ...
        updateEducation: builder.mutation<UpdateEducationResponse, UpdateEducationPayload>({
            query: ({ id, data }) => ({
        ...
        updateSkillSet: builder.mutation<SkillSetResponse, UpdateSkillSetPayload>({
            query: ({ id, data }) => ({
