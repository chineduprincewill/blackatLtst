...
export const Feeds = () => {
    ...
    const [totalPosts, setTotalPosts] = useState<PostResponseFromApi[]>([]);
    const [renderedPosts, setRenderedPosts] = useState<(PostPayload | null)[]>([]);
    const postScrollableRef = useRef(null);

    useEffect(() => {
        const _postsData = postListResponse?.data || [];
        setTotalPosts(_postsData as PostResponseFromApi[]);
    }, [fulfilledTimeStamp]);

    const isPostEmpty = !postText.trim() && selectedContents.length === 0;

    function renderMorePosts(page: number) {
        console.log('RAN');
        setRenderedPosts((prevRenderedPosts) => {
            // Calculate the starting index for the current page
            const postsPerPage = 5;
            const startIndex = (page - 1) * postsPerPage;
            // Get the posts for the current page from totalPosts
            const nextPosts = totalPosts.slice(startIndex, startIndex + postsPerPage).map((post) => ParsePostPayload(post));
            // Concatenate the new posts with the previously rendered posts
            return [...prevRenderedPosts, ...nextPosts];
        });
    }
    ...
    const submitPost = async () => {
        ...
        await createPost({
            ...
            .then((res) => {
                ...
                setRenderedPosts((prev) => [ParsePostPayload(res.data), ...prev]);
    ...
    return (
        <div className="w-full post_area " style={{ position: 'relative', zIndex: 1 }}>
            {showmodal && (
                <ModalContainer>
            ...
            <div
                className={`feed_main pane pt-4 md:!pt-5 px-1 sm:px-2 md:px-0 overflow-x-hidden`}
                id="feeds_container"
                style={{ position: 'relative' }}
                ref={postScrollableRef}
            >
                ...
                <div className="w-full posts ">
                    ...
                    <InfiniteScroll
                        loader={<p onClick={() => renderMorePosts(1)}>Loading...</p>}
                        loadMore={renderMorePosts}
                        threshold={250}
                        useWindow={false}
                        hasMore={renderedPosts.length < totalPosts.length}
                        getScrollParent={() => postScrollableRef.current}
                    >
                        ...
                        {renderedPosts.map((post) => (
                            <Post
                                key={post?.id}
                                // @ts-ignore
                                onAddPostToFeed={(post) => setRenderedPosts((prev) => [ParsePostPayload(post), ...prev])}
                                onDeletePost={() => setTotalPosts((prev) => prev.filter((p) => p?.id !== post?.id))}
                                // @ts-ignore
                                post={post}
                                // @ts-ignore
            ...
            <RepostDialog onAddPost={(post) => setRenderedPosts((prev) => [ParsePostPayload(post), ...prev])} />
            <ReportPostDialog />
        </div>
    );
};
