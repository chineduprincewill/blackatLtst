...
export const generateHmacSHA256 = (message: string, secret: string) => {
    ...
    return window.crypto.subtle
        .importKey('raw', key, { name: 'HMAC', hash: { name: 'SHA-256' } }, true, ['sign'])
        .then((cryptoKey) => {
            return window.crypto.subtle.sign('HMAC', cryptoKey, data);
        })
        .then((signature) => {
            const arrayBuffer = new Uint8Array(signature);
            const hexString = Array.from(arrayBuffer, (byte) => byte.toString(16).padStart(2, '0')).join('');
            return hexString;
        });
};
const IntercomWidget = () => {
    ...
    useEffect(() => {
        ...
        if (user && window.Intercom) {
            generateHmacSHA256(user.toString(), secretKey).then((item) => {
